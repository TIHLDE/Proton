	...
	const processedDayEvents = useMemo(() => {
		const result = days.map((day) => {
			...
			const sortedEvents = [...dayEvents].sort((a, b) => {
				...
				const aEnd = new Date(a.endAt || a.startAt);
				const bEnd = new Date(b.endAt || b.startAt);
			...
			const columns: { event: TeamEvent; end: Date }[][] = [];
					...
					if (col.length === 0) {
						...
						const overlaps = col.some((c) =>
							areIntervalsOverlapping(
								...
								{
									start: new Date(c.event.startAt),
									end: new Date(c.event.endAt || c.event.startAt),
								},
							),
						);
						if (!overlaps) {
